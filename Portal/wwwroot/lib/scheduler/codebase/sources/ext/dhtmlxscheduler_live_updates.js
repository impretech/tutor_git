...
if (typeof(dataProcessor) !== "undefined") {
    ...
    dataProcessor.prototype.init = function () {
        ...
        this.attachEvent("onAfterUpdate", function (sid, action, tid, tag) {
            ...
            var item;
            if (self.obj.exists(tid))
                item = self.obj.item(tid);
			else if (self.obj.exists(sid))
...
if (typeof(scheduler) !== "undefined") {
	...
	scheduler.update = function(id, data) {
		...
		this.updateEvent(id);
		this.callEvent("onEventChanged", [id, event]);
	};
	scheduler.remove = function(id, data) {
		if (this.exists(id)){

			var ev = this.getEvent(id);
			if(this._get_rec_markers){
				if (ev.rec_type)
	...
	scheduler.add = function(data) {
		var id = this.addEvent(data.start_date, data.end_date, data.text, data.id, data);
		if(this._is_modified_occurence && this._is_modified_occurence(data))
			this.setCurrentView();
		return id;
	};
