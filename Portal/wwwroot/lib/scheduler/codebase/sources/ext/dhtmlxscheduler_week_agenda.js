...
scheduler.attachEvent("onTemplatesReady", function() {
	...
	var fillWeekAgendaTab = function() {
		...
		for (var i = 0; i < 7; i++) {
			...
			scheduler._waiAria.weekAgendaDayCell(day_divs[i], tstart);
	...
	scheduler.week_agenda_view = function(mode) {
		...
		if (mode) { // mode enabled
			...
			if(scheduler._wa._prev_data_border === undefined)
				scheduler._wa._prev_data_border = scheduler._els['dhx_cal_data'][0].style.borderTopWidth;
			scheduler._els['dhx_cal_data'][0].style.borderTopWidth = 0;
			...
			var materialScalePlaceholder = scheduler.$container.querySelector(".dhx_cal_scale_placeholder");
			if(materialScalePlaceholder){
                materialScalePlaceholder.style.display = 'none';
			}
			...
			if (scheduler._wa._prev_data_border !== undefined){
				scheduler._els['dhx_cal_data'][0].style.borderTopWidth = scheduler._wa._prev_data_border;
				delete scheduler._wa._prev_data_border;
			}
