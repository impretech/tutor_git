...
scheduler.createTimelineView=function(obj){
	...
	scheduler._merge(obj,{
		...
		_x_scroll: 0,
		_y_scroll: 0,
		_h_cols: [], // [{div: htmlElem, left: number}, ... {}]
...
scheduler._timeline_calculate_event_positions = function(evs){
	if (evs && this.render != "cell"){
		...
		if(maxOrderEvent){
			scheduler.render_timeline_event.call(this, maxOrderEvent, false);
		}
	}
};
...
/* get html section */
// cell mode
scheduler._timeline_get_html_for_cell_data_row = function(i, stats, top_pos) {
	return "<div class='dhx_timeline_data_row' data-section-id='" + i + "' style='" + stats.summ_width + stats.style_height + " position:absolute; top:" + (top_pos) + "px;'>";
};
...
scheduler._timeline_get_html_for_cell = function(x_ind, y_ind, view, ev, stats, cellLeftPos) {
	return "<div data-col-id='"+x_ind+"' class='dhx_matrix_cell dhx_timeline_data_cell " +
		scheduler.templates[view.name + "_cell_class"](ev, view._trace_x[x_ind], view.y_unit[y_ind]) +
		"' style='width:" + (scheduler._cols[x_ind]) + "px;" + (stats.style_height) + stats.style_line_height + "  left:" + (cellLeftPos) + "px;'>" +
		"<div style='width:auto'>" + scheduler.templates[view.name + "_cell_value"](ev, view._trace_x[x_ind], view.y_unit[y_ind]) +
		"</div></div>";
};

// bar mode
scheduler._timeline_get_html_for_bar_matrix_line = function(i, stats, top_pos) {
	return "<div style='" + stats.summ_width + " " + stats.style_height + " position:absolute; top:"+top_pos+"px;' data-section-id='"+i+"' class='dhx_matrix_line'>";
};
...
scheduler._timeline_get_html_for_bar = function(x_ind, y_ind, view, ev, cellLeftPos) {
	return "<div class='dhx_matrix_cell dhx_timeline_data_cell " +
		scheduler.templates[view.name + "_cell_class"](ev, view._trace_x[x_ind], view.y_unit[y_ind]) +
		"' style='width:" + (scheduler._cols[x_ind]) + "px; left:" + (cellLeftPos) + "px;'  data-col-id='"+ x_ind +"'></div>";
};
...
function initScroll(d, view, heights, render_stats) {
	...
	dataWrapperDiv.onscroll = function(e) {
		...
		for (var i= 0, len = rows.length; i < len; i++) {
			var rowIndex = rows[i].getAttribute("data-section-id");
			unit = view.y_unit[rowIndex];

			heights[rowIndex] = render_stats[rowIndex].height;

			//heights.push(rows[i].offsetHeight);
			var unit_key = unit.key;
			var scale = view._scales[unit_key] = (scheduler._isRender('cell')) ? rows[i] : rows[i];
	...
	if (view.scroll_position) {
		...
		scheduler._timeline_save_scroll_pos(view, dataWrapperDiv.scrollTop, dataWrapperDiv.scrollLeft);
	} else {
...
scheduler._timeline_y_scale = function y_scale(d) {
	...
	for (var i=0; i<this.y_unit.length; i++){
		var stats = scheduler._timeline_get_cur_row_stats(this, i);

		// calculate height of all events but will render below only events in viewport
		scheduler._timeline_calculate_event_positions.call(this, evs[i]);

		stats = scheduler._timeline_get_fit_events_stats(this, i, stats);

		// now section 1 = labelWrapper, section 2 = dataWrapper
		var labelRow = "<div class='dhx_timeline_label_row "+stats.tr_className+"' style='top:"+cellTopPos+"px;"+stats.style_height + stats.style_line_height+"'"+"data-row-id='"+i+"'>" +
		...
		if (this.scrollable)
			this._label_rows.push({div: labelRow, top: cellTopPos});

		// check vertical direction
		if (scrollableDataContainer){
		...
		if (skipRenderCells) {
			...
			if (scrollableDataContainer){
				scheduler._timeline_smart_render._rendered_labels_cache.push(i);
			}

			// check horizontal direction
			var cellLeftPos = 0;
			if (this.render == "cell") {
				dataWrapper +=scheduler._timeline_get_html_for_cell_data_row(i, stats, cellTopPos-stats.height);
				for (var j = 0; j < scheduler._cols.length; j++) {
					if (scheduler._ignores[j])
						...
						if (scrollableDataContainer) {
							if (scheduler._timeline_smart_render.isInXViewPort({left: cellLeftPos, right: cellLeftPos + scheduler._cols[j]}, viewPort)) {
								dataWrapper += scheduler._timeline_get_html_for_cell(j, i, this, evs[i][j], stats, cellLeftPos);
							}
						} else {
							dataWrapper += scheduler._timeline_get_html_for_cell(j, i, this, evs[i][j], stats, cellLeftPos);
						}
					...
					cellLeftPos += scheduler._cols[j];
				}
				dataWrapper += '</div>';
			} else {
				//section 2
				dataWrapper += scheduler._timeline_get_html_for_bar_matrix_line(i, stats, cellTopPos-stats.height);

				var printableEvents = evs[i];
				// get events only in viewport
				if (scrollableDataContainer) {
					printableEvents = scheduler._timeline_smart_render.getVisibleEventsForRow(this, viewPort, evs, i);
				...
				for (var j = 0; j < scheduler._cols.length; j++) {
					if (scheduler._ignores[j])
						...
						if (scrollableDataContainer) {
							if (scheduler._timeline_smart_render.isInXViewPort({left: cellLeftPos, right: cellLeftPos + scheduler._cols[j]}, viewPort)) {
								dataWrapper += scheduler._timeline_get_html_for_bar(j, i, this, evs[i], cellLeftPos);
							}
						} else {
							dataWrapper += scheduler._timeline_get_html_for_bar(j, i, this, evs[i], cellLeftPos);
						}
	...
	var rows;
	if (this.render === 'cell')
		rows = d.querySelectorAll('.dhx_timeline_data_col .dhx_timeline_data_row');
	else
		rows = d.querySelectorAll('.dhx_timeline_data_col .dhx_matrix_line');
	...
	for (var i= 0, len = render_stats.length; i < len; i++) {
		...
		//heights.push(rows[i].offsetHeight);
		var unit_key = unit.key;
		var scale = this._scales[unit_key] = (scheduler._isRender('cell')) ? rows[i] : rows[i];
...
scheduler._timeline_x_scale = function x_scale(h){
	...
	for (var i=0; i<this._trace_x.length; i++){
		...
		this._h_cols.push({div: clnLastChild, left: left[i]});
	}
...
scheduler._locate_cell_timeline = function(e){
	...
	var isScale = false;
	while (res.x === 0 &&  scheduler._getClassName(trg) != "dhx_cal_data" && trg.parentNode) {
		if (scheduler._getClassName(trg).split(" ")[0] == "dhx_matrix_scell") {
			isScale = true;
			break;
		} else {
			trg = trg.parentNode;
...
scheduler._timeline_smart_render = {
	...
	getVisibleHeader: function(view, viewPort) {
		if (!view._h_cols.length) return;

		var curHeader = '';
		this._rendered_header_cache = [];

		for (var i = 0; i < view._h_cols.length; i++) {
			if (this.isInXViewPort({left: view._h_cols[i].left, right: view._h_cols[i].left + scheduler._cols[i]}, viewPort)) {
				var html = view._h_cols[i].div.outerHTML;
				curHeader += html;

				this._rendered_header_cache.push(i);
			}
		}
	...
	updateHeader: function(view, viewPort, parent) {
		...
		var visibleItems = this._rendered_header_cache.slice();
	...
	_deleteLabelCells: function(items, view, parent) {
		for (var i = 0; i < items.length; i++) {
			var item = parent.querySelector('[data-row-id="'+items[i]+'"]');
			parent.removeChild(item);
		}
	},
	...
	updateEvents: function(view, viewPort) {
		...
		for (var i = 0; i < this._rendered_labels_cache.length; i++) {
			...
			var line = grid.querySelector('[data-section-id="'+ row +'"]');
	...
	_addEvents: function(events, view, parent, i) {
		...
		parent.insertAdjacentHTML('afterBegin', events_html);
	},
	...
	getVisibleGridColsHTML: function(view, viewPort, stats, evs, i) {
		...
		if (view.render == "cell") {
			for (var ind = 0; ind < this._rendered_header_cache.length; ind++) {
				var j = this._rendered_header_cache[ind];
	...
	getVisibleGridRowsHTML: function(view, viewPort, evs, i) {
		var dataWrapper = '';
		...
		// check vertical direction
		if (view.render == "cell") {
			dataWrapper += scheduler._timeline_get_html_for_cell_data_row(i, stats, view._label_rows[i].top);
			dataWrapper += this.getVisibleGridColsHTML(view, viewPort, stats, evs, i);
			dataWrapper += '</div>';
		} else {
			//section 2
			dataWrapper += scheduler._timeline_get_html_for_bar_matrix_line(i, stats, view._label_rows[i].top);
	...
	_deleteGridRows: function(items) {
		...
		for (var i = 0; i < items.length; i++) {
			var item = parent.querySelector('[data-section-id="'+(items[i])+'"]');
			parent.removeChild(item);
		}
	...
	_addGridRows: function(items, view, viewPort) {
		...
		var html = '';
		for (var i = 0; i < items.length; i++) {
			html += this.getVisibleGridRowsHTML(view, viewPort, evs, items[i]);
		}
	...
	updateGridCols: function(view, viewPort) {
		if (this._cols_to_delete.length) {
			this._deleteGridCols(this._cols_to_delete, view);
		}
		if (this._cols_to_add.length) {
			this._addGridCols(this._cols_to_add, view, viewPort);
		}
	},
	...
	_deleteGridCols: function(items, view) {
		...
		for (var r = 0; r < this._rendered_labels_cache.length; r++) {
			...
			if (view.render == 'cell') {
				parent = grid.querySelector('[data-section-id="'+(this._rendered_labels_cache[r])+'"]');
			} else {
				parent = grid.querySelector('[data-section-id="'+(this._rendered_labels_cache[r])+'"] .dhx_timeline_data_row ');
			}
	...
	_addGridCols: function(items, view, viewPort) {
		...
		for (var r = 0; r < this._rendered_labels_cache.length; r++) {
			...
			if (view.render == 'cell') {
				parent = grid.querySelector('[data-section-id="'+ i +'"]');
			} else {
				parent = grid.querySelector('[data-section-id="'+ i +'"] .dhx_timeline_data_row');
			}
	...
	getVisibleGridCell: function(view, viewPort, stats, evs, i, j) {
		if (!view._h_cols[j]) return;
		var dataWrapper = '';
		var cellLeftPos = view._h_cols[j].left-view.dx;
