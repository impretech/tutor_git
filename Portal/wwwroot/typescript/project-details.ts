...
export class ProjectDetailsList {
    ...
    private init(): void {
        ...
        this.NewProjectButton = document.querySelector("#new-project-button") as HTMLButtonElement;
    ...
    private PortfolioGridSelectionChanged = (arg: kendo.ui.GridChangeEvent): void => {
...
export class ProjectDetailsItem {
    ...
    constructor(data: any) {
        ...
        //sendMessage
        let msg = document.querySelector(".sendMsgAdditionalPanel") as HTMLAnchorElement;
        ...
        const clsBtn = document.querySelector(".closeAdditionalPanel") as HTMLAnchorElement;
    ...
    private async getMessagesByItem(item, itemNo, projectId) {
    ...
    private init(): void {
        ...
        const projectNotesButton = document.querySelector("#project-notes-button") as HTMLButtonElement;
        ...
        const hideNotesButton = document.querySelector("#hideNotes") as HTMLButtonElement;
        ...
        const refreshLocMil = document.querySelector("#refresh-location-milestone") as HTMLAnchorElement;
        ...
        window.addEventListener('attachupdate', (e: CustomEvent) => {
    ...
    async BuildMilestones(): Promise<void> {
        ...
        if (this.data.project.projectId !== 0) {      
            ...
            let hasSchedule = await axios.get<boolean>("/api/Project/IsScheduleCreated?projid=" + this.data.project.projectId);
            if (hasSchedule) {
    ...
    private BuildTeamMemberCards(teamData: any) {
    ...
    private setupNewContactCombo(e: any) {
        ...
        this.AddMemberButton = document.getElementById("addMemberBut") as HTMLButtonElement;
    ...
    private LoadDocumentGrid(): void {
        ...
        $("#document-details-grid").kendoGrid({
            ...
            itemType = dataItem.itemType as string;
            var itemNumber = dataItem.itemNo as string;
        ...
        $("#action-details-grid").kendoGrid({
            ...
            var dataItem = grid.dataItem(cell.closest("tr")) as any;
            var from = dataItem.from as string;
            var message = dataItem.messageID as string;

            itemType = "Action";
            var itemID = dataItem.itemID as string;
            var pm = 'None';
            var second = '';
            var first = '';
            var tracking = '';
          ...
          /*  const trackingResponse = await axios.get("api/project/GetTrackingData?POLineID=" + message);
            ...
            const mss = document.querySelector("#sendMsg") as HTMLAnchorElement;
            mss.addEventListener("click", () => {
                $("#actionPanel").css("display", "block");
            });
            this.UploadDocButton = document.querySelector(".doc-buttonPanel") as HTMLDivElement;
            this.UploadDocButton.addEventListener("click", () => {
        ...
        $("#purchaseOrder-details-grid").kendoGrid({
            ...
            var dataItem = grid.dataItem(cell.closest("tr")) as any;
            itemType = "POLineItem";
            var qty = dataItem.qty as string;
            var des = dataItem.description as string;
            var ordDate = kendo.toString(kendo.parseDate(dataItem.orderDate as string), "MM/dd/yyyy");
            var dueDate = kendo.toString(kendo.parseDate(dataItem.dueDate as string), "MM/dd/yyyy");
            var vendor = dataItem.vendor as string;
            var itemID = dataItem.itemID as string;
            var pono = dataItem.poLineItemID as string;
            var pn = dataItem.poNo as string;
            var pm = 'None';
            var second = '';
            var first = '';
            var tracking = '';
        ...
        $("#fieldReports-details-grid").kendoGrid({
            ...
            var dataItem = grid.dataItem(cell.closest("tr")) as any;
            var images_gird = dataItem.images as Array<any>;

            itemType = "Action";
            var itemID = dataItem.fr.fReportID as string;
            var images = '';
            var it = 0;
            var cl = '';
    ...
    private LoadFinancialGrid(): void {
        let itemType: string;
        ...
        $("#budget-details-grid").kendoGrid({
            ...
            category = dataItem.category as string;
    ...
    private LoadScheduleGrid(): void {
        ...
        $("#schedule-details-grid").kendoGrid({
            ...
            var task = dataItem.task as string;
    ...
    private LoadScheduleChart(): void {
        ...
        if (this.data.project.projectId !== 0) {
            ...
            scheduleData = scheduleTable.data() as any;
    ...
    private GridSelectionChanged = (arg: kendo.ui.GridChangeEvent): void => {
    ...
    private async LoadPreviewPanel(grid: string, selectedItem: any): Promise<void> {
    ...
    private PopulatePreviewPanel(project: any, grid: string, panel: JQuery<HTMLElement>): void {
    ...
    private ClearAllOtherGridSelections(id: string) {
        let grids = [
            "estimate-grid",
            "budget-grid",
            "deposit-grid",
    ...
    private async getDocByUrl(docId: number) {
        let docUrl = await axios.get("api/document/GetFile?id=" + docId);
        return docUrl.data;
    }
}