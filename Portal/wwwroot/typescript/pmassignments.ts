...
export class PMAssignments {
    private inCompleteProjects: any;
    private projectVal: any;
    private toAssignPmId: any;
    private selectedPmId: any;
    private selectedPId: any;
    ...
    assignPmBtn(e) {
        if (this.toAssignPmId !== "")
        {
            axios
                .post(
                    `/api/project/AssignPM?projId=${this.selectedPId}&PMId=${this.toAssignPmId}`,
                    null
                )
                .then(res => {
                    let notification = new Notification();
                    notification.ShowNotification("Save Success!", "PM assigned successfully.", "success");
                    var currentPm = document.getElementById("current-pm") as HTMLInputElement;
                    var assignedPm = document.getElementById("assigned-pm") as HTMLInputElement;
    ...
    removePmBtn(e) {
        ...
        if (this.toAssignPmId !== "")
        {
            let currentPm = document.getElementById("current-pm") as HTMLInputElement;
            currentPm.value = "";
        ...
        if (this.selectedPmId == pmSelected) // If the PM in the current PM box is actual PM of the project, then we send the request to the server to remove it
        {
            axios
                .post(
                    `/api/project/UnAssignPm?projId=${this.selectedPId}&PMId=${this.selectedPmId}`,
                    null
                )
                .then(res => {
                    if (res.data)
                    {
                        const assignedPM = document.getElementById("assigned-pm") as HTMLInputElement;
                        const currentPM = document.getElementById("current-pm") as HTMLInputElement;
                        assignedPM.value = "";
                        currentPM.value = "";
                        this.selectedPmId = "";
                        let notification = new Notification();
    ...
    projectChange(e) {
        ...
        const doc = document;
        const dateFrom = doc.getElementById("dateFrom") as HTMLInputElement;
        const dateTo = doc.getElementById("dateTo") as HTMLInputElement;
        const dFrom = doc.getElementById("d-from") as HTMLInputElement;
        const dTo = doc.getElementById("d-to") as HTMLInputElement;
        const assignedPm = doc.getElementById("assigned-pm") as HTMLInputElement;
        const currentPm = doc.getElementById("current-pm") as HTMLInputElement;
        const projectType = doc.getElementById("project-type") as HTMLInputElement;
        const extValue = doc.getElementById("ext-value") as HTMLInputElement;
        const title = doc.getElementById("p-title") as HTMLInputElement;
        this.projectVal = value;

        if (value !== "") {
            const project = this.inCompleteProjects.find(
                x => String(x.projectId) === value
            );
    ...
    loadPMs = pId => {
        ...
        for (const loc of locations) {
            const l = loc as HTMLInputElement;
            if (l.checked) locList = locList + l.value + ",";
        }

        for (const skill of skills) {
            const s = skill as HTMLInputElement;
            if (s.checked) skillList = skillList + s.value + ",";
        }

        axios
            .get(
                `/api/project/pm-suggestions?projectId=${pId}&skillFilters=${skillList}&locationFilter=${locList}`
            )
            .then(res => {
    ...
    appendPMs = (pmList,pId) => {        
        ...
        pmList.forEach((pm, index) => {
            ...
            if (isCurrentPm) {
                html = `
<div class="col-2 borderd-grey cell-pad h-150px" style=" ${
                    index % 2 === 0 ? "background:#f5f5f5" : ""
                    }">${pm.name}<br/><img src='/images/pm.png' alt='pm' style="width:35px;" /></div>
<div class="col-10 borderd-grey h-150px" style=" ${
                    index % 2 === 0 ? "background:#ebf8fa" : ""
                    }"><div class="graph-div" id="graph-${pm.pmId}" style="width:100%;padding:0px;margin:0px;"></div></div>
`;
            }
            else {
                html = `
<div class="col-2 borderd-grey cell-pad h-150px" style=" ${
                    index % 2 === 0 ? "background:#f5f5f5" : ""
                    }"><a href="javascript:void(0)" id="pm-${pm.pmId}" class="pmNewAssign" style="cursor:pointer;" >${
                    pm.name 
                    }</a></div>
<div class="col-10 borderd-grey h-150px" style=" ${
                    index % 2 === 0 ? "background:#ebf8fa" : ""
...
`;
    ...
    getIncompleteProjects() {
        axios
            ...
            .then(res => {
                if (res.status === 200) {                   
                    const projectDB = document.getElementById("project-list") as HTMLSelectElement;
