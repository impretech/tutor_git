...
export class App {
    ...
    private pushpin: HTMLElement;
    private rightPane: HTMLElement;
    private rightSidebar: HTMLElement;
    private rightPushpin: HTMLElement;
    private menuItems: NodeListOf<Element>;
    private searchBar: HTMLElement;
    private email: any;
    private unreadedMsg: number;
    private mail: string;

    private scrollbar: PerfectScrollbar;
    private rightScrollbar: PerfectScrollbar;
    ...
    private async getMessages() {
        ...
        unreadedMsg.data.forEach((item, index) => {
            ...
            var doc = document.querySelector("#msgID" + item.messageID) as HTMLAnchorElement;
    ...
    private async buildMessages(data: any) {

        let container = document.querySelector(".outlook-365 > .content") as HTMLElement;
        container.innerHTML = "";
        ...
        for (const msg of data) {
            ...
            anchor.addEventListener("click", async (evt) => {
                ...
                let a = evt.currentTarget as HTMLAnchorElement;
            ...
            let divEmail = document.createElement("div") as HTMLDivElement;
            ...
            let divTS = document.createElement("div") as HTMLDivElement;
            divTS.classList.add("timestamp");
            divTS.innerHTML = Utilities.FormatDateTimeString(msg.dateRec);

            let divMessage = document.createElement("div") as HTMLDivElement;
    ...
    private async getMessage(msgId: string) {
...
//        if (item.itemType == 'Email') {
  ...
  //        }
        ...
        let sendBtn = document.querySelector("#viewBtn" + item.messageID) as HTMLButtonElement;
    ...
    private async getDocUrl(docId: string) {
        let docUrl = await axios.get("api/document/GetFile?id=" + docId);
        window.open(docUrl.data);
        document.body.classList.toggle("wait");
    }

    private async buildDocs(data: any) {

        let container = document.querySelector(".doc-portal > .content") as HTMLElement;
        container.innerHTML = "";
        ...
        for (const doc of data) {
            ...
            let divInfo = document.createElement("div") as HTMLDivElement;
            divInfo.classList.add("file-info");

            let divName = document.createElement("div") as HTMLDivElement;
            divName.classList.add("file-name");
            divName.innerHTML = doc.name;
            divInfo.appendChild(divName);

            let divSize = document.createElement("div") as HTMLDivElement;
        ...
        if (this.menuItems.length > 0) {
            ...
            for (const menuItem of this.menuItems) {
                menuItem.addEventListener('click', (event) => {
                    var link = event.currentTarget as HTMLElement;
    ...
    private loadMenuConfig(): void {
        if (localStorage.getItem("menu") !== null) {
            ...
            for (const menuItem of menuArray) {
                const link = document.getElementById(menuItem) as HTMLElement;
                if (link != null) {
            ...
            const first = document.querySelector(".main-menu a") as HTMLElement;
    ...
    private storeMenuConfig(id: string, add: boolean): void {
    ...
    private loadDockingConfig(): void {
    ...
    private storeDockingConfig(val: string): any {
        localStorage.setItem("docked", val);
    }

    private storeRightDockingConfig(val: string): any {
        localStorage.setItem("right-docked", val);
    }

    private pushPinMouseLeave(e: any): void {
    ...
    private navMouseLeave(): void {
        document.querySelector(".sidebar").classList.remove("visible");
    }

    private navMouseEnter(): void {
    ...
    private turnOnTransitions(): void {
        setTimeout(() => {
    ...
    private setupSearchBar(): void {
        document.querySelector(".search-bar-button").addEventListener("click", (event) => {
            ...
            let input = document.querySelector(".search-bar-input") as HTMLInputElement;
            input.focus();
        });
