...
export class ProjectControlCenter {
    constructor() {
        this.AlertsGridSelectionChanged = (arg) => {
            let selectedItem = arg.sender.dataItem(arg.sender.select());
            this.LoadAlertsPanel(selectedItem.projectId);
        };
        this.PortfolioGridSelectionChanged = (arg) => {
            ...
            if (keys.length === 1) {
                this.LoadAlertsPanel(parseInt(keys[0]));
            }
            else {
    ...
    setupGrids() {
        ...
        $("#alerts-grid").kendoGrid({
            ...
            if (cellIndex >= 4 && cellIndex <= 7) {
                ...
                dataItem = grid.dataItem(cell.closest("tr"));
                this.LoadAlertsPanel(dataItem.id, column.title);
            }
            else {
                dataItem = grid.dataItem(cell.closest("tr"));
                this.LoadAlertsPanel(dataItem.id);
            }
        });
    ...
    LoaddocumentDetailGrid(itemType) {
    ...
    async LoadAlertsPanel(projectId, columnName) {
        document.querySelector(".alerts-panel-container").classList.remove("visible");
    ...
    PopulateAlertsPanelDetail(project, alerts, panel, forceCategory) {
        ...
        let desc = detailPane.querySelector(".description textarea");
        desc.value = project.project.description;
        let note = detailPane.querySelector(".notes textarea");
        if (project.lastNote) {
